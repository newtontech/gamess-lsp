# Agent Verification Loop for GAMESS LSP

## Overview

This document describes the recommended verification loop for coding agents
(Claude Code, OpenCode, Codex) working with GAMESS input files using the
GAMESS LSP server.
...
The GAMESS LSP provides custom commands for agent consumption:

- `GAMESS.diagnosticsSnapshot` — Returns JSON snapshot of all diagnostics.
...
```python
config = TestRunnerConfig(executable="GAMESS", enabled=True)
