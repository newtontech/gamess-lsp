...
def _load_intent(path: Path) -> dict[str, Any] | None:
    ...
    (e.g. ``software_version``, ``mwords_warning``). It is a workspace-local
    artifact, never a MatMaster/Bohrium runtime concept.
    """
    case_dir = path if path.is_dir() else path.parent
    intent_path = case_dir / ".gamess-lsp" / "intent.json"
...
def _looks_like_workspace(path: Path) -> bool:
    ...
    Preflight needs a parseable GAMESS input to build a meaningful cross-artifact
    graph; a directory with no ``.inp`` falls back to the legacy single-file
    lint path so callers that progressively build inputs are not flooded with
...
def _resolve_input_path(path: Path) -> Path:
    ...
    if path.is_dir():
        # Prefer an explicit .inp, otherwise the first file with a $ group.
        candidates = sorted(p for p in path.iterdir() if p.is_file())
        for candidate in candidates:
            if candidate.suffix.lower() == ".inp":
                return candidate
        from .preflight import looks_like_gamess_workspace

        for candidate in candidates:
            if looks_like_gamess_workspace(candidate):
                return candidate
...
def manifest_path(path: Path | None = None) -> dict[str, Any]:
    ...
    ``.gamess-lsp/fixtures.json`` are merged in so the parent probe can confirm
    ...
    if path is not None:
        case_dir = path if path.is_dir() else path.parent
        fixtures_path = case_dir / ".gamess-lsp" / "fixtures.json"
