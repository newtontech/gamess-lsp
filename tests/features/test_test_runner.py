...
from gamess_lsp.features.test_runner import (
...
class TestConfig:
    ...
    def test_ok(self):
        assert len(TestRunnerConfig(executable="rungms", enabled=True).validate()) == 0
...
class TestProvider:
    ...
    def test_snapshot(self):
        s = json.loads(TestRunnerProvider(TestRunnerConfig(executable="rungms", enabled=True)).snapshot_config())
        assert s["enabled"]
