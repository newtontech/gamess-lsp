#!/usr/bin/env python3
"""Install a GAMESS-LSP wheel in isolation and smoke its public CLIs."""
...
def smoke(wheel: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="gamess-lsp-wheel-") as temporary:
        ...
        server = bindir / f"gamess-lsp{executable_suffix}"
        tool = bindir / f"gamess-lsp-tool{executable_suffix}"
        ...
        installed_version = _run(
            [
                str(python),
                "-c",
                "from importlib.metadata import version; print(version('gamess-lsp'))",
            ],
            cwd=isolated,
        ).stdout.strip()
        ...
        valid = ROOT / "tests/fixtures/valid_water_dft.inp"
        ...
        invalid = ROOT / "tests/fixtures/invalid_unclosed_group.inp"
        ...
        log = ROOT / "tests/fixtures/gamess_output/convergence_failure.log"
        log_payload = _json(_run([str(tool), "log", str(log)], cwd=isolated))
        if log_payload.get("capabilities", {}).get("operation") != "log":
            raise RuntimeError("runtime-log fixture did not exercise the agent log path")
        if log_payload.get("has_errors") is not True or not log_payload.get("diagnostics"):
            raise RuntimeError("runtime-log fixture did not produce error diagnostics")
